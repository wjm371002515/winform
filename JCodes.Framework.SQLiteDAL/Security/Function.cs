...
namespace JCodes.Framework.SQLiteDAL
{
    ...
    /// 系统功能定义
    /// </summary>
    public class Functions : BaseDALSQLite<FunctionInfo>, IFunctions
    {
        #region 对象实例及构造函数
        ...
        public Functions()
            : base(SQLitePortal.gc._securityTablePre+"Function", "ID")
        {
        ...
        public List<FunctionInfo> GetFunctions(string roleIDs, string typeID)
        {
            ...
            INNER JOIN {0}Role_Function On [{0}Function].ID={0}Role_Function.Function_ID WHERE Role_ID IN ({1})", SQLitePortal.gc._securityTablePre, roleIDs);
            if (typeID.Length > 0)
            {
        ...
        {
            string sql = string.Format( @"SELECT * FROM [{0}Function] 
            LEFT JOIN {0}Role_Function On [{0}Function].ID={0}Role_Function.Function_ID WHERE Role_ID = {1}",SQLitePortal.gc._securityTablePre, roleID);
            return this.GetList(sql, null);
        }
        ...
        public List<FunctionNodeInfo> GetTreeWithRole(string systemType, List<int> roleList)
        {
            ...
            if (roleList.Count > 0)
            {
                ...
                WHERE RF.Role_ID IN ({1}) AND F.SystemType_ID = '{2}' Order By PID, Name ", SQLitePortal.gc._securityTablePre, roleString, systemType);
