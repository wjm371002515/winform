...
namespace JCodes.Framework.SQLiteDAL
{
    ...
    /// 登陆系统的黑白名单列表
    /// </summary>
    public class BlackIP : BaseDALSQLite<BlackIPInfo>, IBlackIP
    {
        #region 对象实例及构造函数
        ...
        {
            string commandText = string.Format("DELETE FROM {0}BLACKIP_USER WHERE BLACKIP_ID='{1}'", SQLitePortal.gc._securityTablePre, blackID);
            Database db = CreateDatabase();
            DbCommand command = db.GetSqlStringCommand(commandText);
            db.ExecuteNonQuery(command);
        }
