...
namespace JCodes.Framework.SQLiteDAL
{
    ...
    /// 角色信息
    /// </summary>
    public class Role : BaseDALSQLite<RoleInfo>, IRole
    {
        #region 对象实例及构造函数
        ...
        public Role()
            : base(SQLitePortal.gc._securityTablePre+"Role", "ID")
        {
        ...
        {
            string commandText = string.Format("INSERT INTO {0}Role_Function(Function_ID, Role_ID) VALUES('{1}',{2})",SQLitePortal.gc._securityTablePre, functionID, roleID);
        ...
        {
            string commandText = string.Format("INSERT INTO {0}OU_Role(OU_ID, Role_ID) VALUES({1},{2})",SQLitePortal.gc._securityTablePre, ouID, roleID);
            Database db = CreateDatabase();
            DbCommand command = db.GetSqlStringCommand(commandText);
            db.ExecuteNonQuery(command);
        }
        ...
        {
            string commandText = string.Format("INSERT INTO {0}User_Role(User_ID, Role_ID) VALUES({1},{2})", SQLitePortal.gc._securityTablePre, userID, roleID);
            Database db = CreateDatabase();
            DbCommand command = db.GetSqlStringCommand(commandText);
            db.ExecuteNonQuery(command);
        }
        ...
        {
            string sql = string.Format("Delete from {0}User_Role where Role_ID = {1} ",SQLitePortal.gc._securityTablePre, roleID);
            base.SqlExecute(sql);
        ...
        {
            string sql = string.Format("Delete from {0}Role_Function where Role_ID = {1} ", SQLitePortal.gc._securityTablePre, roleID);
            base.SqlExecute(sql);
        ...
        {
            string sql = string.Format("Delete from {0}OU_Role where Role_ID = {1} ", SQLitePortal.gc._securityTablePre, roleID);
            base.SqlExecute(sql);
        ...
        public List<RoleInfo> GetRolesByFunction(string functionID)
        {
            ...
            INNER JOIN {0}Role_Function On {0}Role.ID={0}Role_Function.Role_ID WHERE Function_ID ='{1}' ", SQLitePortal.gc._securityTablePre, functionID);
            return this.GetList(sql, null);
        }

        public List<RoleInfo> GetRolesByOU(int ouID)
        {
            string sql = string.Format("SELECT * FROM {0}Role INNER JOIN {0}OU_Role ON {0}Role.ID=Role_ID WHERE OU_ID = {1}", SQLitePortal.gc._securityTablePre, ouID);
            return this.GetList(sql, null);
        }

        public List<RoleInfo> GetRolesByUser(int userID)
        {
            string sql = string.Format("SELECT * FROM {0}Role INNER JOIN {0}User_Role On {0}Role.ID={0}User_Role.Role_ID WHERE User_ID = {1}", SQLitePortal.gc._securityTablePre, userID);
            return this.GetList(sql, null);
        }

        public void RemoveFunction(string functionID, int roleID)
        {
            string commandText = string.Format("DELETE FROM {0}Role_Function WHERE Function_ID='{1}' AND Role_ID={2}", SQLitePortal.gc._securityTablePre, functionID, roleID);
            Database db = CreateDatabase();
            DbCommand command = db.GetSqlStringCommand(commandText);
            db.ExecuteNonQuery(command);
        }
        ...
        {
            string commandText = string.Format("DELETE FROM {0}OU_Role WHERE OU_ID={1} AND Role_ID={2}", SQLitePortal.gc._securityTablePre, ouID, roleID);
            Database db = CreateDatabase();
            DbCommand command = db.GetSqlStringCommand(commandText);
            db.ExecuteNonQuery(command);
        }
        ...
        {
            string commandText = string.Format("DELETE FROM {0}User_Role WHERE User_ID={1} AND Role_ID={2}", SQLitePortal.gc._securityTablePre, userID, roleID);
            Database db = CreateDatabase();
            DbCommand command = db.GetSqlStringCommand(commandText);
            db.ExecuteNonQuery(command);
        }
