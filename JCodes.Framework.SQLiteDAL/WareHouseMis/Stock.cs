...
namespace JCodes.Framework.SQLiteDAL
{
	...
	/// Stock 的摘要说明。
	/// </summary>
	public class Stock : BaseDALSQLite<StockInfo>, IStock
	{
		#region 对象实例及构造函数
        ...
        public Stock()
            : base(SQLitePortal.gc._wareHouseTablePre + "Stock", "ID")
		{
		}
        ...
        public DataTable GetCurrentStockReport(string condition)
        {
            ...
            string sql = string.Format(@"Select t.ID,d.ItemNo,d.ItemName,Price,t.StockQuantity,(Price * t.StockQuantity) as StockAmount,d.Manufacture,d.MapNo,d.Specification,d.Material,d.ItemBigType,d.ItemType,d.Unit, Source,StoragePos,UsagePos,LowWarning,HighWarning,t.Note,t.WareHouse,d.Dept
                                         From {0}Stock t inner join {0}ItemDetail d on t.ItemNo = d.ItemNo  {1} order by t.id ", SQLitePortal.gc._wareHouseTablePre, condition);
            return this.SqlTable(sql);
        }

        public int GetCurrentStockReportCount(string condition)
        {
            string sql = string.Format(@"Select count(*) From {0}Stock t inner join {0}ItemDetail d on t.ItemNo = d.ItemNo  {1} ", SQLitePortal.gc._wareHouseTablePre, condition);
