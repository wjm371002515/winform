...
namespace JCodes.Framework.SQLiteDAL
{
	...
	/// City 的摘要说明。
	/// </summary>
    public class City : BaseDALSQLite<CityInfo>, ICity
	{
		#region 对象实例及构造函数
		...
		public City() : base(SQLitePortal.gc._basicTablePre+"City","ID")
		{
            IsDescending = false;
		}
        ...
        {
            string sql = string.Format("Select c.* from {0}City as c inner join {0}Province as p on c.ProvinceId=p.ID where ProvinceName='{1}' ", SQLitePortal.gc._basicTablePre, provinceName);
