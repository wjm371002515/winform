...
namespace JCodes.Framework.SQLiteDAL
{
	...
	/// DictType 的摘要说明。
	/// </summary>
    public class DictType : BaseDALSQLite<DictTypeInfo>, IDictType
	{
		#region 对象实例及构造函数
        ...
        public DictType()
            : base(SQLitePortal.gc._basicTablePre + "DictType", "ID")
		{
            sortField = "Seq";
            IsDescending = false;
		}
        ...
        public Dictionary<Int32, string> GetAllType(Int32 PID)
        {
            string sql = string.Format("select ID, Name from {0}DictType where PID ={3} order by {1} {2}",
                SQLitePortal.gc._basicTablePre, sortField, IsDescending ? "DESC" : "ASC", PID);
        ...
        public List<DictTypeNodeInfo> GetTree()
        {
            ...
            string sql = string.Format("Select * From {0}DictType Order By PID, Seq ", SQLitePortal.gc._basicTablePre);
            Database db = CreateDatabase();
