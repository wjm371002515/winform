...
namespace JCodes.Framework.MySqlDAL
{
    ...
    /// 通讯录联系人
    /// </summary>
    public class Address : BaseDALMySql<AddressInfo>, IAddress
    {
        #region 对象实例及构造函数
        ...
        public Address()
            : base(MySqlPortal.gc._contactTablePre+"Address", "ID")
        {
        }
        ...
        public override Dictionary<string, string> GetColumnNameAlias()
        {
            ...
            dict.Add("OfficeAddress", "办公电话");
            dict.Add("HomeAddress", "家庭住址");
            dict.Add("OfficeAddress", "办公地址");
            dict.Add("Fax", "传真号码");
            dict.Add("CompanyName", "公司单位");
            dict.Add("DeptName", "部门");
        ...
        {
            //所属用户条件,非删除状态
            string condition = string.Format(" AND t.CreatorId='{0}' ", ownerUser);

            string sql = "";
            if (string.IsNullOrEmpty(groupName))
            {
                sql = string.Format("Select t.* from {1} t where ID not In (select Address_ID from {0}AddressGroup_Address) {2}", MySqlPortal.gc._contactTablePre, tableName, condition);
            }
            else
            {
                string subSql = string.Format("select ID from {0}AddressGroup g where g.Name ='{1}' ", MySqlPortal.gc._contactTablePre, groupName);
                sql = string.Format(@"select t.* from {1} t inner join {0}AddressGroup_Address m on t.ID = m.Address_ID 
                where m.Group_ID in ({2}) {3} ", MySqlPortal.gc._contactTablePre, tableName, subSql, condition);
            }
        ...
        public bool ModifyAddressGroup(Int32 contactId, List<string> groupIdList)
        {
            ...
            if (trans != null)
            {
                string sql = string.Format("Delete from {0}AddressGroup_Address where Address_ID='{1}' ", MySqlPortal.gc._contactTablePre, contactId);
                base.SqlExecute(sql, trans);

                foreach (string groupId in groupIdList)
                {
                    sql = string.Format("Insert into {0}AddressGroup_Address(Address_ID, Group_ID) values('{1}', '{2}') ", MySqlPortal.gc._contactTablePre, contactId, groupId);
                    base.SqlExecute(sql, trans);
                }
