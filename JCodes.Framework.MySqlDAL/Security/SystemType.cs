...
{
    public class SystemType : BaseDALMySql<SystemTypeInfo>, ISystemType
    {
        #region 对象实例及构造函数
        ...
        public bool VerifySystem(string serialNumber, string typeID, int authorizeAmount)
        {
            ...
            bool flag = false;
            string sql = string.Format("SELECT Count(ID) As Records FROM {0}SystemAuthorize WHERE SystemType_OID='{1}' ",MySqlPortal.gc._securityTablePre, typeID);
            command = db.GetSqlStringCommand(sql);
            int num = Convert.ToInt32(db.ExecuteScalar(command).ToString());
            if (num <= authorizeAmount)
            {
                sql = string.Format("SELECT * FROM {0}SystemAuthorize WHERE Content='{1}'  And SystemType_OID='{2}' ", MySqlPortal.gc._securityTablePre, serialNumber, typeID);
                ...
                if (!flag)
                {
                    ...
                    if (flag)
                    {
                        sql = string.Format("INSERT INTO {0}SystemAuthorize (SystemType_OID,Content) VALUES ('{1}', '{2}') ", MySqlPortal.gc._securityTablePre, typeID, serialNumber);
                        command = db.GetSqlStringCommand(sql);
