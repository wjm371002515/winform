...
namespace JCodes.Framework.MySqlDAL
{
    ...
    public class User : BaseDALMySql<UserInfo>, IUser
    {
        ...
        public List<SimpleUserInfo> GetSimpleUsersByOU(int ouID)
        {
            ...
            Inner Join {0}OU_User ON {1}.ID=User_ID Where Deleted = 0 AND {0}OU_User.OU_ID = {2}",MySqlPortal.gc._securityTablePre, tableName, ouID);
            return FillSimpleUsers(sql);
        }
        ...
        {
            string sql = string.Format(@"Select ID,Name,Password,FullName,HandNo,MobilePhone,Email From {1} 
            INNER JOIN {0}User_Role ON {1}.ID=User_ID Where Deleted = 0 AND {0}User_Role.Role_ID ={2} ",MySqlPortal.gc._securityTablePre, tableName, roleID);
        ...
        public List<UserInfo> GetUsersByOU(int ouID)
        {
            ...
            WHERE Deleted = 0 AND {0}OU_User.OU_ID ={2} ", MySqlPortal.gc._securityTablePre, tableName, ouID);
        ...
        public List<UserInfo> GetUsersByRole(int roleID)
        {
            ...
            WHERE Deleted = 0 AND {0}User_Role.Role_ID = {2}", MySqlPortal.gc._securityTablePre, tableName, roleID);
