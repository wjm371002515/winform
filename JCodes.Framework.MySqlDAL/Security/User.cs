...
//    /// 系统用户信息
//    /// </summary>
//    public class User : BaseDALMySql<UserInfo>, IUser
//    {
//        #region 对象实例及构造函数
...
//        }
//        public User()
//            : base(MySqlPortal.gc._securityTablePre + "User", "Id")
//        {
//            this.sortField = "Seq";
...
//            Inner Join {0}OU_User ON {1}.ID=User_ID Where IsDelete = 0 AND {0}OU_User.OU_ID = {2}", MySqlPortal.gc._securityTablePre, tableName, ouID);
//            return FillSimpleUsers(sql);
//        }
...
//        {
//            string sql = string.Format(@"Select ID,Name,Password,FullName,UserCode,MobilePhone,Email From {1} 
//            INNER JOIN {0}User_Role ON {1}.ID=User_ID Where IsDelete = 0 AND {0}User_Role.Role_ID ={2} ", MySqlPortal.gc._securityTablePre, tableName, roleID);
...
//            WHERE IsDelete = 0 AND {0}OU_User.OU_ID ={2} ", MySqlPortal.gc._securityTablePre, tableName, ouID);
...
//            WHERE IsDelete = 0 AND {0}User_Role.Role_ID = {2}", MySqlPortal.gc._securityTablePre, tableName, roleID);
