...
namespace JCodes.Framework.MySqlDAL
{
    ...
    /// 登陆系统的黑白名单列表
    /// </summary>
    public class BlackIP : BaseDALMySql<BlackIPInfo>, IBlackIP
    {
        #region 对象实例及构造函数
        ...
        public BlackIP()
            : base(MySqlPortal.gc._securityTablePre+"BlackIP", "ID")
        {
        ...
        public string GetUserIdList(string id)
        {
            ...
            ON m.BLACKIP_ID=t.ID WHERE t.ID = '{1}' ", MySqlPortal.gc._securityTablePre, id);
            return SqlValueList(sql);
        }

        public void AddUser(int userID, string blackID)
        {
            string commandText = string.Format("INSERT INTO {0}BLACKIP_USER(User_ID, BLACKIP_ID) VALUES({1}, '{2}')",MySqlPortal.gc._securityTablePre, userID, blackID);
        ...
        {
            string commandText = string.Format("DELETE FROM {0}BLACKIP_USER WHERE User_ID={1} AND BLACKIP_ID='{2}'",MySqlPortal.gc._securityTablePre, userID, blackID);
            Database db = CreateDatabase();
            DbCommand command = db.GetSqlStringCommand(commandText);
            db.ExecuteNonQuery(command);
        }
        ...
        {
            string commandText = string.Format("DELETE FROM {0}BLACKIP_USER WHERE BLACKIP_ID='{1}'", MySqlPortal.gc._securityTablePre, blackID);
            Database db = CreateDatabase();
            DbCommand command = db.GetSqlStringCommand(commandText);
            db.ExecuteNonQuery(command);
        }
        ...
        public List<BlackIPInfo> FindByUser(int userId, AuthrizeType type)
        {
            ...
            ON t.ID=m.BLACKIP_ID WHERE m.User_ID = {1} and t.AuthorizeType={2} and t.Forbid=0 ", MySqlPortal.gc._securityTablePre, userId, (int)type);
