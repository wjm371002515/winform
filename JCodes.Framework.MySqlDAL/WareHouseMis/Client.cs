...
namespace JCodes.Framework.MySqlDAL
{
	...
	/// WareHouse 的摘要说明。
	/// </summary>
    public class Client : BaseDALMySql<ClientInfo>, IClient
	{
		#region 对象实例及构造函数
        ...
        public Client()
            : base(MySqlPortal.gc._wareHouseTablePre + "Client", "ID")
        {
            this.sortField = "ID";
        ...
        {
            string sql = string.Format("select ID, Code+'('+Name+')'  as Name from {0}Client order by ID", MySqlPortal.gc._wareHouseTablePre);
