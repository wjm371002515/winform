...
namespace JCodes.Framework.CommonControl
{
    ...
    /// </remarks>
    internal class CoolPrintPreviewControl : DevExpress.XtraEditors.XtraUserControl
    {
        #region ** fields
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            if (img != null)
            {
                ...
                if (rc.Width > 2 && rc.Height > 2)
                {
                    ...
                    {
                        // render first page
                        rc.Width = (rc.Width - Const.MARGIN) / 2;
                        RenderPage(e.Graphics, img, rc);
                        ...
                        if (img != null)
                        {
                            ...
                            // render second page
                            rc.Offset(rc.Width + Const.MARGIN, 0);
                            RenderPage(e.Graphics, img, rc);
                        }
                    }
                }
        ...
        Rectangle GetImageRectangle(Image img)
        {
            ...
            rc.Inflate(-Const.MARGIN, -Const.MARGIN);
            if (_zoomMode == ZoomMode.TwoPages)
            {
                rc.Inflate(-Const.MARGIN / 2, 0);
            }
        ...
        void UpdateScrollBars()
        {
            ...
            {
                case ZoomMode.PageWidth:
                    scrollSize = new Size(0, rc.Height + 2 * Const.MARGIN);
                    break;
                case ZoomMode.ActualSize:
                case ZoomMode.Custom:
                    scrollSize = new Size(rc.Width + 2 * Const.MARGIN, rc.Height + 2 * Const.MARGIN);
                    break;
            }
