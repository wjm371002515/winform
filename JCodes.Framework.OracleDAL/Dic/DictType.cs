...
namespace JCodes.Framework.OracleDAL
{
	...
	/// DictType 的摘要说明。
	/// </summary>
    public class DictType : BaseDALOracle<DictTypeInfo>, IDictType
	{
		#region 对象实例及构造函数
        ...
        public DictType()
            : base(OraclePortal.gc._basicTablePre + "DICTTYPE", "ID")
		{
            sortField = "Seq";
            IsDescending = false;
        ...
        public Dictionary<Int32, string> GetAllType(Int32 PID)
        {
            string sql = string.Format("select ID, Name from {0}DictType where PID ={3} order by {1} {2}",
                OraclePortal.gc._basicTablePre, sortField, IsDescending ? "DESC" : "ASC", PID);
        ...
        public List<DictTypeNodeInfo> GetTree()
        {
            ...
            string sql = string.Format("Select * From {0}DictType Order By PID, Seq ", OraclePortal.gc._basicTablePre);
            Database db = CreateDatabase();
