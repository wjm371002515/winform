...
namespace JCodes.Framework.OracleDAL
{
    ...
    public class Menus : BaseDALOracle<MenuInfo>, IMenus
    {
        ...
        public Menus()
            : base(OraclePortal.gc._securityTablePre + "Menu", "Gid")
        {
            this.sortField = "Seq";
        ...
        public List<MenuNodeInfo> GetTree(string systemType)
        {
            ...
            string sql = string.Format("Select * From {0} Where IsVisable > 0 {1} Order By Gid, Seq ", tableName, condition);

            DataTable dt = base.SqlTable(sql);
            string sort = string.Format("{0} {1}", GetSafeFileName(sortField), isDescending ? "DESC" : "ASC");
            DataRow[] dataRows = dt.Select(string.Format(" Gid = '{0}' ", -1), sort);
        ...
        {
            string sql = string.Format(@"Select t.*,decode(Pgid, '-1', 0, Pgid) as parentId From {1} t 
