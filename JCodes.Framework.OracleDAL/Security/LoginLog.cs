...
namespace JCodes.Framework.OracleDAL
{
    ...
    /// 用户登录日志信息
    /// </summary>
    public class LoginLog : BaseDALOracle<LoginLogInfo>, ILoginLog
    {
        #region 对象实例及构造函数
        ...
        {
            string sql = string.Format("Select * from {0} where UserId='{1}' order by LastUpdateTime desc where  rownum <= 2", tableName, userId);
