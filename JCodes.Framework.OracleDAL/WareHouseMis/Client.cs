...
namespace JCodes.Framework.OracleDAL
{
	...
	/// WareHouse 的摘要说明。
	/// </summary>
    public class Client : BaseDALOracle<ClientInfo>, IClient
	{
		#region 对象实例及构造函数
        ...
        public Client()
            : base(OraclePortal.gc._wareHouseTablePre + "Client", "ID")
        {
        ...
        {
            string sql = string.Format("select ID, Code||'('||Name||')'  as Name from {0}Client order by ID", OraclePortal.gc._wareHouseTablePre);
