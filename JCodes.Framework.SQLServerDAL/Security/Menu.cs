...
namespace JCodes.Framework.SQLServerDAL
{
    ...
    public class Menus : BaseDALSQLServer<MenuInfo>, IMenus
    {
        ...
        public Menus()
            : base(SQLServerPortal.gc._securityTablePre + "Menu", "Gid")
        {
            this.sortField = "Seq";
        ...
        public List<MenuNodeInfo> GetTree(string systemType)
        {
            ...
            string sql = string.Format("Select * From {0} Where Visible > 0 {1} Order By Pgid, Seq ", tableName, condition);
        ...
        {
            string condition = !string.IsNullOrEmpty(systemType) ? string.Format("AND SystemtypeId='{0}'", systemType) : "";
            string sql = string.Format("Select * From {0} Where Visible > 0 and Pgid='-1' {1} Order By Seq  ", tableName, condition);
            return GetList(sql, null);
        }
        ...
        {
            List<MenuNodeInfo> arrReturn = new List<MenuNodeInfo>();
            string sql = string.Format("Select * From {0} Where Visible > 0 Order By Pgid, Seq ", tableName);
