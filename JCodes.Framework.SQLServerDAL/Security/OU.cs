...
namespace JCodes.Framework.SQLServerDAL
{
    ...
    /// 机构（部门）信息
    /// </summary>
    public class OU : BaseDALSQLServer<OUInfo>, IOU
    {
        #region 系统所需函数
        #region 对象实例及构造函数
        ...
        public OU()
            : base(SQLServerPortal.gc._securityTablePre + "OU", "ID")
        {
        ...
        {
            string sql = string.Format("Delete from {0}OU_User where OU_ID = {1} ", SQLServerPortal.gc._securityTablePre, ouID);
            base.SqlExecute(sql);
        ...
        {
            string commandText = string.Format("INSERT INTO {0}OU_User(User_ID, OU_ID) VALUES({1},{2})", SQLServerPortal.gc._securityTablePre, userID, ouID);
        ...
        {
            string commandText = string.Format("DELETE FROM {0}OU_User WHERE User_ID={1} AND OU_ID={2}", SQLServerPortal.gc._securityTablePre, userID, ouID);
            Database db = CreateDatabase();
            DbCommand command = db.GetSqlStringCommand(commandText);
            db.ExecuteNonQuery(command);
        }
        ...
        public override bool DeleteByUser(object key, string userId, DbTransaction trans = null)
        {
            ...
            if (info != null)
            {
                string sql = string.Format("UPDATE [{0}OU] SET PID={1} Where PID={2}", SQLServerPortal.gc._securityTablePre, info.PID, key);
                SqlExecute(sql, trans);

                sql = string.Format("Delete From [{0}OU] Where ID={1}", SQLServerPortal.gc._securityTablePre, key);
                SqlExecute(sql, trans);
            }
            return true;
        }
        ...
        {
            string sql = string.Format("SELECT * FROM {0}OU INNER JOIN {0}OU_Role On [{0}OU].ID={0}OU_Role.OU_ID WHERE Role_ID = {1}",  SQLServerPortal.gc._securityTablePre, roleID);
            return this.GetList(sql, null);
        }

        public List<OUInfo> GetOUsByUser(int userID)
        {
            string sql = string.Format("SELECT * FROM {0}OU INNER JOIN {0}OU_User On [{0}OU].ID={0}OU_User.OU_ID WHERE User_ID = {1}" , SQLServerPortal.gc._securityTablePre, userID);
            return this.GetList(sql, null);
        }
