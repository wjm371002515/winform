...
namespace JCodes.Framework.SQLServerDAL
{
	...
	/// </summary>
    public class PurchaseHeader : BaseDALSQLServer<PurchaseHeaderInfo>, IPurchaseHeader
	{
		#region 对象实例及构造函数
        ...
        public PurchaseHeader()
            : base(SQLServerPortal.gc._wareHouseTablePre + "PurchaseHeader", "ID")
		{
		}
        ...
        public List<string> GetYearList(string fieldName)
        {
            ...
            string sql = string.Format("select distinct Year({0}) from {1} order by Year({0}) ", fieldName, tableName);
        ...
        public int GetPurchaseQuantity(string condition, bool isIn)
        {
            ...
            INNER JOIN {0}PurchaseHeader AS h ON d.PurchaseHead_ID = h.ID Where {1} AND {2}", SQLServerPortal.gc._wareHouseTablePre, subWhere, condition);
        ...
        public DataTable GetPurchaseReport(string condition)
        {
            ...
            from {0}PurchaseHeader h inner join {0}PurchaseDetail d on h.ID = d.PurchaseHead_ID {1} order by h.CreateDate", SQLServerPortal.gc._wareHouseTablePre, condition);
