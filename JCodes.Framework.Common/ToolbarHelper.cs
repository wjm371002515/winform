        ...
        public static string GetToolBar(DataTable dt, string pageName)
        {
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                switch (dt.Rows[i]["Code"].ToString())
                {
                    ...
                    // 吴建明 新增
                    case "search":
                        sb.Append("{\"text\": \"" + dt.Rows[i]["Name"] + "\",\"iconCls\":\"" + dt.Rows[i]["Icon"] + "\",\"handler\":\"" + pageName + "_setsearch();\"},");
                        break;
                    case "batchoperate":
                        sb.Append("{\"text\": \"" + dt.Rows[i]["Name"] + "\",\"iconCls\":\"" + dt.Rows[i]["Icon"] + "\",\"handler\":\"" + pageName + "_batchoperate();\"},");
                        break;
                    default:
                        //browser不是按钮
                        break;
