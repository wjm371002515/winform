...
namespace JCodes.Framework.Common
{
    ...
    public class NetworkUtil
    {
        ...
        public static List<KeyValuePair<string, string>> GetNetCardList()
        {
            ...
            try
            {
                RegistryKey regNetCards = Registry.LocalMachine.OpenSubKey(Const.REG_NET_CARDS_KEY);
                if (regNetCards != null)
                {
        ...
        public static string GetPhysicalAddr(string cardId)
        {
            ...
            try
            {
                ...
                device = Win32Utils.CreateFile(driveName,
                                         ...
                                         0, Const.OPEN_EXISTING, 0, 0);
                if (device != Const.INVALID_HANDLE_VALUE)
                {
                    byte[] outBuff = new byte[6];
                    uint bytRv = 0;
                    int intBuff = Const.PERMANENT_ADDRESS;

                    if (0 != Win32Utils.DeviceIoControl(device, Const.IOCTL_NDIS_QUERY_GLOBAL_STATS,
        ...
        public static int GetValidPort(string port)
        {
            ...
            if (validPort <= Const.MINPORT || validPort > Const.MAXPORT)
            {
