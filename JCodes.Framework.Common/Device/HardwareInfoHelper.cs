    ...
    public sealed class HardwareInfoHelper
    {
        ...
        internal sealed class CpuUsageNt : CpuUsage
        {
            ...
            public CpuUsageNt()
            {
                ...
                int ret;
                // get new system time
                ret = NtQuerySystemInformation(Const.SYSTEM_TIMEINFORMATION, timeInfo, timeInfo.Length, IntPtr.Zero);
                if (ret != Const.NO_ERROR)
                    throw new NotSupportedException();
                // get new CPU's idle time
                ret = NtQuerySystemInformation(Const.SYSTEM_PERFORMANCEINFORMATION, perfInfo, perfInfo.Length, IntPtr.Zero);
                if (ret != Const.NO_ERROR)
                    throw new NotSupportedException();
                // get number of processors in the system
                ret = NtQuerySystemInformation(Const.SYSTEM_BASICINFORMATION, baseInfo, baseInfo.Length, IntPtr.Zero);
                if (ret != Const.NO_ERROR)
                    throw new NotSupportedException();
        ...
        private static HardDiskInfo GetHddInfoNT(byte driveIndex)
        {
            ...
            IntPtr hDevice = CreateFile(
             ...
             Const.GENERIC_READ | Const.GENERIC_WRITE,
             Const.FILE_SHARE_READ | Const.FILE_SHARE_WRITE,
             IntPtr.Zero,
             Const.OPEN_EXISTING,
             0,
             IntPtr.Zero);
