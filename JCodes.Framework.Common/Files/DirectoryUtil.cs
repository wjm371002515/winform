...
namespace JCodes.Framework.Common.Files
{
    ...
    public class DirectoryUtil
    {
        ...
        {
            string[] lst = null;
            lst = Util.Merge<string>(lst, GetFileNames(directoryPath));

            string[] tmpdir = GetDirectories(directoryPath);
            for (Int32 i = 0; i < tmpdir.Length; i++)
            {
                lst = Util.Merge<string>(lst, GetAllFileNames(tmpdir[i]));
            }
            return lst;
        }
