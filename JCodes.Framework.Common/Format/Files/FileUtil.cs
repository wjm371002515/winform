...
namespace JCodes.Framework.Common.Files
{
    ...
    public class FileUtil
    {
        ...
        public static byte[] StreamToBytes(Stream stream)
        {
            ...
            {
                LogHelper.WriteLog(LogLevel.LOG_LEVEL_CRIT, ex, typeof(FileUtil));
                throw ex;
            }
            finally
            {
        ...
        public static void Rename(string sourceFile, string descFile) {
        ...
        /// 文件合并
        /// </summary>
        /// <param name="filenames">需要合并的文件集合</param>
        /// <param name="mergeFileName">合并之后的文件名</param>
        public static void MergeFiles(string[] filenames, string mergeFileName)
        {
            Int32 iSumFile = filenames.Length;
            FileStream AddStream = new FileStream(mergeFileName, FileMode.OpenOrCreate);
            ...
            //文件长度和文件内容叠加
            {
                ...
                {
                    TempStream = new FileStream(filenames[i], FileMode.Open);
            ...
            //创建含有头信息的文件- 也达到了加密的效果
            {
                ...
                AddStream = new FileStream(mergeFileName + "_r", FileMode.OpenOrCreate);
                AddWriter = new BinaryWriter(AddStream);
                //文件头
                AddWriter.Write(BytesMessage);
 
                //写入原来文件
                TempStream = new FileStream(mergeFileName, FileMode.Open);
                TempReader = new BinaryReader(TempStream);
                AddWriter.Write(TempReader.ReadBytes((int)TempStream.Length));
            ...
            foreach (var file in filenames) {
                if (FileUtil.IsExistFile(file) && !FileUtil.FileIsUsing(file)) {
