...
namespace JCodes.Framework.Data.BLL
{
    ...
    {
        private static readonly IUserDepartment dal = Factory.GetUserDepartmentDAL();
        ...
        {
            DataTable dt_user_dep_old = new Department().GetDepartmentByUserId(userId);  //用户之前拥有的部门
            List<Model.UserDepartment> dep_addList = new List<Model.UserDepartment>();     //需要插入部门的sql语句集合
            List<Model.UserDepartment> dep_deleteList = new List<Model.UserDepartment>();     //需要删除部门的sql语句集合

            string[] str_dep = depIds.Trim(',').Split(',');    //传过来用户勾选的部门（有去勾的也有新勾选的）

            Model.UserDepartment userdepdelete = null;
            Model.UserDepartment userdepadd = null;
            for (int i = 0; i < dt_user_dep_old.Rows.Count; i++)
            {
                ...
                {
                    userdepdelete = new Model.UserDepartment();
            ...
            if (!string.IsNullOrEmpty(depIds))
            {
                for (int j = 0; j < str_dep.Length; j++)
                {
                    ...
                    {
                        userdepadd = new Model.UserDepartment();
