...
namespace JCodes.Framework.Data.BLL
{
    ...
    public class User
    {
        private static readonly IUser dal = Factory.GetUserDAL();
        ...
        public string GetPager(string tableName, string columns, string order, int pageSize, int pageIndex, string where, out int totalCount)
        {
            ...
            {
                //这么写多连了20*2次数据库，待改进
                DataTable dtrole = new Role().GetRoleByUserId(Convert.ToInt32(dt.Rows[i]["Id"]));
                DataTable dtdepartment = new Department().GetDepartmentByUserId(Convert.ToInt32(dt.Rows[i]["Id"]));
        ...
        {
            Model.User userCompare = dal.GetUserByUserId(user.UserId);
            if (userCompare != null)
            {
