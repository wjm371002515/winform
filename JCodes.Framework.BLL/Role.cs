...
namespace JCodes.Framework.Data.BLL
{
    ...
    public class Role
    {
        private static readonly IRole dal = Factory.GetRoleDAL();
        ...
        {
            Model.Role roleCompare = dal.GetRoleByRoleName(role.RoleName);
            if (roleCompare != null)
            {
        ...
        public bool Authorize(int roleId, string roleMenuButtonId)
        {
            ...
            List<Model.RoleMenuButton> role_menu_button_addlist = new List<Model.RoleMenuButton>();    //需要新添加的（用户新勾选的）
            List<Model.RoleMenuButton> role_menu_button_deletelist = new List<Model.RoleMenuButton>();   //需要删除的（用户去除勾选的）
            ...
            for (int i = 0; i < dt_role_menu_button_old.Rows.Count; i++)
            {
                ...
                {
                    Model.RoleMenuButton roleMenuButton = new Model.RoleMenuButton();
                ...
                for (int i = 0; i < role_menu_button_userselect.Length; i++)
                {
                    ...
                    if (dt_role_menu_button_old.Select(string.Format("MenuId={0} and ButtonId={1}", menuId, buttonId)).Length == 0)
                    {
                        ...
                        if (parentId != -1 && !listParentMenuId.Contains(parentId))
                        {
                            ...
                            Model.RoleMenuButton roleParentMenu = new Model.RoleMenuButton();
