...
namespace JCodes.Framework.Data.BLL
{
    ...
    {
        private static readonly IUserRole dal = Factory.GetUserRoleDAL();
        ...
        {
            DataTable dt_user_role_old = new Role().GetRoleByUserId(userId);  //用户之前拥有的角色
            List<Model.UserRole> role_addList = new List<Model.UserRole>();     //需要插入角色的sql语句集合
            List<Model.UserRole> role_deleteList = new List<Model.UserRole>();     //需要删除角色的sql语句集合

            string[] str_role = roleIds.Trim(',').Split(',');    //传过来用户勾选的角色（有去勾的也有新勾选的）

            Model.UserRole userroledelete = null;
            Model.UserRole userroleadd = null;
            //用户去掉勾选的角色（要删除本用户的角色）
            for (int i = 0; i < dt_user_role_old.Rows.Count; i++)
            {
                ...
                {
                    userroledelete = new Model.UserRole();
            ...
            if (!string.IsNullOrEmpty(roleIds))
            {
                for (int j = 0; j < str_role.Length; j++)
                {
                    ...
                    {
                        userroleadd = new Model.UserRole();
