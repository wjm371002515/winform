...
namespace JCodes.Framework.AccessDAL
{
    ...
    /// 通讯录联系人
    /// </summary>
    public class Address : BaseDALAccess<AddressInfo>, IAddress
    {
        #region 对象实例及构造函数
        ...
        public Address()
            : base(AccessPortal.gc._contactTablePre+"Address", "ID")
        {
        }
        ...
        public List<AddressInfo> FindByGroupName(string ownerUser, string groupName, PagerInfo pagerInfo = null)
        {
            ...
            {
                sql = string.Format("Select t.* from {1} t where ID not In (select Address_ID from {0}AddressGroup_Address) {2}", AccessPortal.gc._contactTablePre, tableName, condition);
            }
            else
            {
                string subSql = string.Format("select ID from {0}AddressGroup g where g.Name ='{1}' ", AccessPortal.gc._contactTablePre, groupName);
                sql = string.Format(@"select t.* from {1} t inner join {0}AddressGroup_Address m on t.ID = m.Address_ID 
                where m.Group_ID in ({2}) {3} ", AccessPortal.gc._contactTablePre, tableName, subSql, condition);
            }
        ...
        public bool ModifyAddressGroup(string contactId, List<string> groupIdList)
        {
            ...
            if (trans != null)
            {
                string sql = string.Format("Delete from {0}AddressGroup_Address where Address_ID='{1}' ", AccessPortal.gc._contactTablePre, contactId);
                base.SqlExecute(sql, trans);

                foreach (string groupId in groupIdList)
                {
                    sql = string.Format("Insert into {0}AddressGroup_Address(Address_ID, Group_ID) values('{1}', '{2}') ", AccessPortal.gc._contactTablePre, contactId, groupId);
                    base.SqlExecute(sql, trans);
                }
