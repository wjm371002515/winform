...
namespace JCodes.Framework.AccessDAL
{
    ...
    public class Functions : BaseDALAccess<FunctionInfo>, IFunctions
    {
        ...
        public Functions()
            : base(AccessPortal.gc._securityTablePre + "Function", "ID")
        {
        ...
        public List<FunctionInfo> GetFunctions(string roleIDs, string typeID)
        {
            ...
            INNER JOIN {0}Role_Function On [{0}Function].ID={0}Role_Function.Function_ID WHERE Role_ID IN ({1})", AccessPortal.gc._securityTablePre, roleIDs);
            if (typeID.Length > 0)
            {
        ...
        {
            string sql = string.Format(@"SELECT * FROM [{0}Function] 
            LEFT JOIN {0}Role_Function On [{0}Function].ID={0}Role_Function.Function_ID WHERE Role_ID = {1}",AccessPortal.gc._securityTablePre, roleID);
            return this.GetList(sql, null);
        }
        ...
        public List<FunctionNodeInfo> GetTreeByID(string mainID)
        {
            ...
            DataTable dt = SqlTable(sql);
            string sortCode = string.Format("{0} {1}", GetSafeFileName(sortField), isDescending ? "DESC" : "ASC");
            DataRow[] dataRows = dt.Select(string.Format(" PID = '{0}'", mainID), sortCode);
            for (int i = 0; i < dataRows.Length; i++)
            {
                string id = dataRows[i]["ID"].ToString();
                FunctionNodeInfo menuNodeInfo = GetNode(id, dt);
        ...
        public List<FunctionNodeInfo> GetTreeWithRole(string systemType, List<int> roleList)
        {
            ...
            if (roleList.Count > 0)
            {
                ...
                WHERE RF.Role_ID IN ({1}) AND F.SystemType_ID = '{2}' Order By PID, Name ", AccessPortal.gc._securityTablePre, roleString, systemType);
