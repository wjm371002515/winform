...
namespace JCodes.Framework.AccessDAL
{
    ...
    /// 系统用户信息
    /// </summary>
    public class User : BaseDALAccess<UserInfo>, IUser
    {
        #region 对象实例及构造函数
        ...
        {
            string sql = string.Format("Select ID,Name,Password,FullName,UserCode,MobilePhone,Email From {0}OU_User, [{0}User] Where [{0}User].ID=User_ID And IsDelete = 0 AND OU_ID = {1}", AccessPortal.gc._securityTablePre, ouID);
            return FillSimpleUsers(sql);
        }

        public List<SimpleUserInfo> GetSimpleUsersByRole(int roleID)
        {
            string sql = string.Format("Select ID,Name,Password,FullName,UserCode,MobilePhone,Email From [{0}User], {0}User_Role Where [{0}User].ID=User_ID and IsDelete = 0 AND Role_ID = {1}", AccessPortal.gc._securityTablePre, roleID);
