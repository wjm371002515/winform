...
namespace JCodes.Framework.AccessDAL
{
    ...
    /// 角色信息
    /// </summary>
    public class Role : BaseDALAccess<RoleInfo>, IRole
    {
        #region 对象实例及构造函数
        ...
        public Role()
            : base(AccessPortal.gc._securityTablePre+"Role", "ID")
        {
            this.sortField = "Seq";
        ...
        {
            string commandText = string.Format("INSERT INTO {0}Role_Function(Function_ID, Role_ID) VALUES('{1}',{2})",AccessPortal.gc._securityTablePre, functionID, roleID);
        ...
        {
            string commandText = string.Format("INSERT INTO {0}OU_Role(OU_ID, Role_ID) VALUES({1},{2})", AccessPortal.gc._securityTablePre, ouID, roleID);
            Database db = CreateDatabase();
            DbCommand command = db.GetSqlStringCommand(commandText);
            db.ExecuteNonQuery(command);
        }
        ...
        {
            string commandText = string.Format("INSERT INTO {0}User_Role(User_ID, Role_ID) VALUES({1},{2})", AccessPortal.gc._securityTablePre, userID, roleID);
            Database db = CreateDatabase();
            DbCommand command = db.GetSqlStringCommand(commandText);
            db.ExecuteNonQuery(command);
        }
        ...
        {
            string sql = string.Format("Delete from {0}User_Role where Role_ID = {1} ", AccessPortal.gc._securityTablePre, roleID);
            base.SqlExecute(sql);
        ...
        {
            string sql = string.Format("Delete from {0}Role_Function where Role_ID = {1} ", AccessPortal.gc._securityTablePre, roleID);
            base.SqlExecute(sql);
        ...
        {
            string sql = string.Format("Delete from {0}OU_Role where Role_ID = {1} ", AccessPortal.gc._securityTablePre, roleID);
            base.SqlExecute(sql);
        ...
        {
            string sql = string.Format("SELECT * FROM {0}Role, {0}Role_Function Where {0}Role.ID={0}Role_Function.Role_ID and Function_ID ='{1}' ", AccessPortal.gc._securityTablePre, functionID);
            return this.GetList(sql, null);
        }

        public List<RoleInfo> GetRolesByOU(int ouID)
        {
            string sql = string.Format("SELECT * FROM {0}Role, {0}OU_Role where {0}Role.ID=Role_ID and OU_ID = ", AccessPortal.gc._securityTablePre, ouID);
            return this.GetList(sql, null);
        }

        public List<RoleInfo> GetRolesByUser(int userID)
        {
            string sql = string.Format("SELECT * FROM {0}Role, {0}User_Role Where {0}Role.ID={0}User_Role.Role_ID and User_ID = {1}", AccessPortal.gc._securityTablePre, userID);
            return this.GetList(sql, null);
        }

        public void RemoveFunction(string functionID, int roleID)
        {
            string commandText = string.Format("DELETE FROM {0}Role_Function WHERE Function_ID='{1}' AND Role_ID={2}", AccessPortal.gc._securityTablePre, functionID, roleID);
            Database db = CreateDatabase();
            DbCommand command = db.GetSqlStringCommand(commandText);
            db.ExecuteNonQuery(command);
        }
        ...
        {
            string commandText = string.Format("DELETE FROM {0}OU_Role WHERE OU_ID={1} AND Role_ID={2}", AccessPortal.gc._securityTablePre, ouID, roleID);
            Database db = CreateDatabase();
            DbCommand command = db.GetSqlStringCommand(commandText);
            db.ExecuteNonQuery(command);
        }
        ...
        {
            string commandText = string.Format("DELETE FROM {0}User_Role WHERE User_ID={1} AND Role_ID={2}", AccessPortal.gc._securityTablePre, userID, roleID);
            Database db = CreateDatabase();
            DbCommand command = db.GetSqlStringCommand(commandText);
            db.ExecuteNonQuery(command);
        }
