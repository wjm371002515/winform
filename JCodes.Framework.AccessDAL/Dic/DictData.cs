...
namespace JCodes.Framework.AccessDAL
{
	...
	/// DictData 的摘要说明。
	/// </summary>
    public class DictData : BaseDALAccess<DictDataInfo>, IDictData
	{
		#region 对象实例及构造函数
		...
		public DictData() : base(AccessPortal.gc._basicTablePre+"DictData","ID")
		{
            sortField = "Seq";
            IsDescending = false;
		}
        ...
        public List<DicKeyValueInfo> GetAllDict()
        {
            string sql = string.Format("select d.DicttypeID,d.Name,d.Value from {0}DictData d inner join {0}DictType t on d.DicttypeID = t.ID order by d.{1} {2}",
                AccessPortal.gc._basicTablePre, sortField, IsDescending ? "DESC" : "ASC");
