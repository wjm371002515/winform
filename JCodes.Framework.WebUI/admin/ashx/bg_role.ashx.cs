    ...
    public class bg_role : IHttpHandler
    {
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                ...
                {
                    case "getall":
                        context.Response.Write(new Data.BLL.Role().GetAllRole("1=1"));
                        break;
                    case "search":
                        string strWhere = "1=1";
                        ...
                        int totalCount;   //输出参数
                        string strJson = new Data.BLL.Role().GetPager("tbRole", "Id,RoleName,AddDate,ModifyDate,Description", sort + " " + order, pagesize, pageindex, strWhere, out totalCount);
                    ...
                    case "searchRoleUser":
                        ...
                        string strJsonRoleUser = new Data.BLL.Role().GetPagerRoleUser(roleUserId, sortRoleUser + " " + orderRoleUser, pagesizeRoleUser, pageindexRoleUser);
                        ...
                        break;
                    case "add":
                        if (user != null && new Data.BLL.Authority().IfAuthority("role", "add", user.Id))
                        {
                            ...
                            int roleId = new Data.BLL.Role().AddRole(roleAdd);
                            if (roleId > 0)
                            {
                        ...
                        break;
                    case "edit":
                        if (user != null && new Data.BLL.Authority().IfAuthority("role", "edit", user.Id))
                        {
                            ...
                            if (new Data.BLL.Role().EditRole(roleEdit, originalName))
                            {
                        ...
                        break;
                    case "delete":
                        if (user != null && new Data.BLL.Authority().IfAuthority("role", "delete", user.Id))
                        {
                            int id = Convert.ToInt32(context.Request.Params["id"]);
                            if (new Data.BLL.Role().DeleteRole(id))
                            {
                        ...
                        break;
                    case "authorize":
                        if (user != null && new Data.BLL.Authority().IfAuthority("role", "authorize", user.Id))
                        {
                            ...
                            if (new Data.BLL.Role().Authorize(roleId, menuButtonId))
                            {
