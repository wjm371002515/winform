            ...
            try
            {
                ...
                switch (action)
                {
                    case "search":
                        ...
                        if (ui_loginlog_username.Trim() != "" && !SqlInjection.GetString(ui_loginlog_username))   //防止sql注入
                            strWhere += string.Format(" and UserName like '%{0}%'", ui_loginlog_username.Trim());
                        if (ui_loginlog_userip.Trim() != "" && !SqlInjection.GetString(ui_loginlog_userip))
                            strWhere += string.Format(" and UserIp like '%{0}%'", ui_loginlog_userip.Trim());
                        if (ui_loginlog_city.Trim() != "" && !SqlInjection.GetString(ui_loginlog_city))
                        ...
                        string strJson = new Data.BLL.LoginLog().GetPager(strWhere, sort, order, pageindex, pagesize);
